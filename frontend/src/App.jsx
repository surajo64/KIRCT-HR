...
const App = () => {
  ...
  useEffect(() => {
    const publicPaths = ['/', '/login', '/forgot-password', '/reset-password']; // include base reset path too
    const isResetPath = location.pathname.startsWith('/reset-password');
    // debug navigation issues
    // console.log('Auth check:', { token, path: location.pathname, isResetPath });
    if (!token && !publicPaths.includes(location.pathname) && !isResetPath) {
      navigate('/login');
    }
